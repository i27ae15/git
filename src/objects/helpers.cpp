...
namespace VestObjects {
    ...
    std::ostringstream computefilePath(std::string& objSha1) {
    ...
    std::string writeObject(std::string& fContent, std::string dirRoot, std::string sha1) {
        std::vector<uint8_t> compressedContent = VestFile::compressData(fContent);
    ...
    std::string prepareBlob(std::vector<uint8_t>& fContent) {
